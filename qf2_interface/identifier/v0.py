...
def get_interface(target, r, verbose):
    ...
    if r[1] == 0:
        if r[2] == 0:
            ...
            instance = my_exec('import qf2_interface.bmb7r1_bootloaders.v_'+str(bootloader_hash)+' as x', target, verbose)
        else:
            # BMB7 r1 runtime
            # Decide on packet version interface to use and import the associated module
            instance = my_exec('import qf2_interface.bmb7r1_runtimes.v_'+str(runtime_hash)+' as x', target, verbose)
    else:
        if r[2] == 0:
            # QF2-pre bootloader
            # Decide on packet version interface to use and import the associated module
            instance = my_exec('import qf2_interface.qf2_pre_bootloaders.v_'+str(bootloader_hash)+' as x', target, verbose)
        else:
            # QF2-pre runtime
            # Decide on packet version interface to use and import the associated module
            instance = my_exec('import qf2_interface.qf2_pre_runtimes.v_'+str(runtime_hash)+' as x', target, verbose)
