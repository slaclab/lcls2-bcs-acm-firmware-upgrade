...
#
        ...
        def icap_write(self, value, last=False):
                #15 ......8......0
                #8910.... 012.....
                value = (int('{:08b}'.format((value >> 8) & 0xFF)[::-1], 2) << 8) | int('{:08b}'.format(value & 0xFF)[::-1], 2)
                self.set_byte(24, 0, 5)
                self.set_byte(25, value & 0xFF, 0xFF)
                self.set_byte(26, (value >> 8) & 0xFF, 0xFF)
                if last:
                        d = bytearray(self.WRITE_LENGTH)
                        m = bytearray(self.WRITE_LENGTH)
                        d[24] = 1
